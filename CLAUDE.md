...
- **rtk es macOS-only, y por eso el hook va guardado.** Su fórmula de Homebrew no publica bottle de Linux y su web no es un repo de GitHub, así que el patrón `gh_latest_*` de `lib/binaries.sh` tampoco aplica: **en Linux no hay forma de instalarlo hoy**. Pero `config/claude/settings.json` se symlinkea en *todas* las plataformas, así que declarar el hook a pelo lo ejecutaría en cada llamada Bash de cualquier VPS, contenedor o nodo k8s — salida 127 y `rtk: command not found` en todas. De ahí la guarda: `if command -v rtk …; then exec rtk hook claude; fi`. Donde rtk existe, `exec` preserva su código de salida (importa: un `PreToolUse` puede querer bloquear la llamada); donde no, el hook es un no-op silencioso. Lo cubre `config/claude/settings.test.sh`, que extrae el comando del propio JSON y lo ejecuta con un PATH sin rtk.
...
- **El hook va versionado, y `settings.json` no lleva `skip-worktree`.** Antes sí lo llevaba, para que `rtk init -g --hook-only` —que escribe en `~/.claude/settings.json`, o sea, por el symlink, en el archivo del repo— no ensuciara el árbol. El precio fue peor que el problema: durante meses el archivo versionado quedó congelado mientras la config real de la máquina evolucionaba, y una máquina nueva recibía una config obsoleta sin que nada avisara. **Un archivo con `skip-worktree` es un archivo que dejó de estar respaldado.** Ahora el hook está declarado y guardado en el archivo versionado, así que no hay que correr `rtk init` en ninguna máquina y la marca no hace falta. **No hay un settings de usuario no versionado.** Claude Code lee `~/.claude/settings.json` y, por proyecto, `.claude/settings.local.json`; un `~/.claude/settings.local.json` solo cuenta como settings local del proyecto `~`, no como capa de usuario (confirmado contra la doc de settings y en vivo: sus `env` no llegan a la sesión). Lo específico de una máquina que Claude deba ver en todas partes va en el settings versionado **con guarda**, como los hooks de rtk y de iTerm; lo específico de un proyecto, en el `.claude/settings.local.json` de ese proyecto.
